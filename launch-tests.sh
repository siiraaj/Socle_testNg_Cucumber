...
# update language and timeout
curl  -H "Accept: application/json" -u ${API_KEY}: -X POST -F "deviceLanguageCode=fr_FR" -F "timeout=5400" "${API_ENDPOINT}/api/v2/me/projects/${PROJECT_ID}/config"
...
# store response of files
curl -H "Accept: application/json" -u ${API_KEY}: "${API_ENDPOINT}/api/v2/me/projects/${PROJECT_ID}/files" | python -m json.tool > JSON_FILES.json
...
#ZIP_EXISTS="$(curl -H "Accept: application/json" -u ${API_KEY}: "${API_ENDPOINT}/api/v2/me/projects/${PROJECT_ID}/files" | python parseJsonResponseBitbarAPI.py getFileTEST)"
ZIP_EXISTS="$(cat JSON_FILES.json | python parseJsonResponseBitbarAPI.py getFileTEST)"

echo "ZIP_EXISTS: ${ZIP_EXISTS}"
echo "UPLOAD_TEST: $UPLOAD_TEST"

if [[ ( -z ${UPLOAD_TEST} ) && ( ${ZIP_EXISTS} -ne "null" ) ]]; then
    :
else
    # Set the name of zip and copy correct sh template
    if [[ ( "${ANDROID}" = true ) ]] ; then
        OUT_ZIP=$(./zip_project.sh)
    elif [[ ( "${IOS}" = true ) ]] ; then
        OUT_ZIP=$(./zip_project.sh)
    else
...
#APP_EXISTS="$(curl -H "Accept: application/json" -u ${API_KEY}: "${API_ENDPOINT}/api/v2/me/projects/${PROJECT_ID}/files" | python parseJsonResponseBitbarAPI.py getFileAPP)"
APP_EXISTS="$(cat JSON_FILES.json | python parseJsonResponseBitbarAPI.py getFileAPP)"
...
curl -s -H "Accept: application/json" -u ${API_KEY}: -X POST "${API_ENDPOINT}/api/v2/me/projects/${PROJECT_ID}/runs?usedDeviceGroupId=${DEVICE_GROUP_ID}" | python -m json.tool > JSON_RUNS.json

TESTRUN_ID="$(cat JSON_RUNS.json | python -c 'import sys,json;data=json.loads(sys.stdin.read()); print data["id"]')"
echo "${TESTRUN_ID}"
